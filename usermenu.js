...
(function injectUserMenuCSS(){
  ...
  .user-dropdown .item{
    ...
    color:#c00;text-decoration:none;font-weight:600;text-align:left;border:none;background:none;cursor:pointer
  }
...
function makeUserMenu(displayName="Mi Cuenta"){
  const wrap = document.createElement('div');
  wrap.className = 'user-menu';
  wrap.innerHTML = `
    <button class="user-btn" id="userMenuBtn" aria-haspopup="true" aria-expanded="false">
      <span class="avatar">${(displayName||'U').trim().charAt(0).toUpperCase()}</span>
    ...
    <div class="user-dropdown" role="menu" aria-label="Menú de usuario">
      ...
      <hr/>
      <button class="item" id="logoutBtn" type="button">Salir</button>
    </div>
  `;
  wrap.querySelector('#userMenuBtn')?.addEventListener('click', (e)=>{
    e.preventDefault();
...
(async function main(){
    ...
    try{
      ...
      try {
        ...
        if(snap.exists()){
          ...
          displayName = (f||l) ? `${f} ${l}`.trim() : (d.email? String(d.email).split('@')[0] : '');
        }
      } catch {}
      if(!displayName) displayName = user?.email?.split('@')[0] || 'Mi Cuenta';

      const loginLink = findLoginLink();
      if(loginLink){
        const menu = makeUserMenu(displayName);
  ...
  if(cached){ await renderForUser({ uid: cached }); }
